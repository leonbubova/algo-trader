...
import { ArrowUpIcon, ArrowDownIcon, TrendingUpIcon, SearchIcon, UserIcon, LayoutIcon } from "lucide-react"
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <header className="py-6 mb-12 border-b border-gray-200"> {/* Increased mb-8 to mb-12 */}
        <div className="flex justify-between items-center mb-4"> {/* Added mb-4 to create space above the border */}
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Hello, Leon</h1> {/* Increased font size */}
            <p className="text-sm text-gray-500 mt-2">{currentDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p> {/* Increased margin-top */}
          </div>
          <div className="flex space-x-4"> {/* Increased space between buttons */}
            <Button variant="ghost" size="sm" className="text-gray-500 hover:text-gray-900">
              <UserIcon className="h-5 w-5" /> {/* Slightly increased icon size */}
            </Button>
            <Button variant="ghost" size="sm" className="text-gray-500 hover:text-gray-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="bg-white shadow-sm">
          <CardHeader className="pb-2 pt-4">
            <CardTitle className="flex justify-between items-center text-sm">
              ...
              <Button variant="ghost" size="sm" className="text-xs text-gray-500">What&apos;s happening</Button>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <h2 className="text-lg font-bold leading-tight mt-2 mb-3 text-gray-900">Inflation Eases as Fed Heads Into Policy Meeting, Goldman Warns of Delay in Rate Cuts, OPEC+ Under Pressure Amid Low Oil Prices.</h2>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white shadow-sm">
          <CardHeader className="pb-2 pt-4">
            <CardTitle className="flex justify-between items-center text-sm">
              <span className="font-semibold text-gray-900">Sector performance</span>
              ...
              {sectorPerformance.map((sector, index) => (
                <div key={index} className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    <span className={`text-xs font-semibold ${sector.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <tbody>
                {markets.map((market, index) => (
                  <tr key={index} className="border-t border-gray-100">
                    <td className="py-2 text-xs font-medium text-gray-900">{market.name}</td>
                    <td className="py-2 text-xs text-right text-gray-900">{market.value.toFixed(2)}</td>
                    <td className={`py-2 text-xs text-right ${market.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {market.change >= 0 ? '+' : ''}{market.change.toFixed(2)}
                    </td>
                    <td className={`py-2 text-xs text-right ${market.percentChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {market.percentChange >= 0 ? '+' : ''}{market.percentChange.toFixed(2)}%
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <CardContent>
            ...
            <div className="flex justify-between">
              {['1D', '1W', '1M', '3M', 'YTD', '1Y', 'All'].map((period) => (
                <Button key={period} variant="ghost" size="sm" className="text-xs text-gray-500 hover:text-gray-900 px-1">
                  {period}
                </Button>
              ))}
            </div>
